...
- 成员接口合并多个题材后，`is_new` 必须逐题材解释；有 Y/N 的题材过滤历史成员，全空题材保留全部返回成员，避免误报 missing。
- 涨跌停价目标股票 Universe 为空属于上游前置条件错误，任务必须失败且不得删除该日已有数据。
- 当日 EOD 延迟任务保留成功状态，但进度展示为历史交易日实际完成比例而非 100%，并明确提示今日 EOD 待更新。
- IPO 无涨跌幅限制是合法行情语义，不是质量退化：SSE/SZSE 上市起前 5 个交易日、BSE 上市首个交易日的 Provider 特殊涨跌停值计入 `stk_limit` 覆盖率，并保留原始值和可追溯原因。
- 系统不得仅根据 `99999.99/0` 猜测 exemption。上市日期、交易所与连续交易日历不能共同证明时仍按非法涨跌停值阻断；采集校验和存量 RawCompleteness 使用同一规则。

## Milestone 11.3：安全访问、可信研究与可运维部署（2026-09-24）

- 系统提供单管理员登录、退出、当前用户、强制首次改密和服务器 CLI 密码重置；除健康检查和登录外，所有数据、任务和研究功能必须登录后访问。
- 页面由 Nginx 提供并同源代理 `/api`，Compose 一次启动 migration、backend、worker、scheduler、frontend；数据库继续使用外部 PostgreSQL 17。
- 数据页显示 Worker 心跳、活跃任务、队列数量、调度时间和 Raw/分析/机会最新日期；排队任务可立即取消，运行任务在安全点取消。
- Analysis Identity 排除 Raw 质量和 Provider 运行参数，避免无关配置变化导致 Derived 全量失效；涉及因子、趋势等分析语义的配置变化仍必须产生新 Hash。
...
生产配置不再允许默认或弱管理员密码，也不允许关闭安全 Cookie。登录增加轻量失败锁定，所有失败使用统一凭据错误；认证 Session 活跃时间采用节流写入，默认十分钟更新一次。

## Milestone 12.2 Research V2 正确性（2026-09-25）

研究结果新增目标日持仓市值收益、延迟可执行退出、退出延迟与交易成本后的净收益。原可执行 gross 收益含义保持不变。Analytics 和页面同时展示可执行收益、持仓市值收益和延迟退出收益，明确报告不可退出比例，避免跌停或停牌样本被排除后产生乐观偏差。

## Milestone 12.4 Realtime Kline 优化（2026-09-25）

实时 K 线外部请求纳入统一 Provider 日志。系统按股票有效上市区间和 PIT 停牌状态判定应有交易日，只查询合并后的缺失区间，并对同一代码与区间提供短期进程缓存；返回数据仍不写入 Raw 仓库。

## Milestone 12.5 服务可靠性（2026-09-25）

系统可独立观察 Backend、Worker、Scheduler 和数据库状态，不再用最近任务心跳推断空闲 Worker。收盘数据尚未就绪时，Scheduler 会在当日晚间按固定窗口幂等重试；数据与当前版本分析完成后，同一检查链继续触发 Research 刷新。

## Milestone 12.6 工程可维护性（2026-09-25）

系统启动时必须拒绝包含未知字段的 Strategy 配置，CI 必须同时验证后端覆盖率、前端覆盖率、数据库迁移和容器构建。生产 Python 容器以无特权账号运行，降低容器进程被利用后的宿主风险。上述约束不改变股票池、因子、状态机或机会评分语义。

## Milestone 12.7 Research 与调度正确性（2026-09-25）

Research V3 统一股票退出完整性规则，在停牌日以目标日前最近有效价格估值，并保留估值来源日期。研究批次必须覆盖最大 Horizon 及延迟退出窗口。自动调度对同一交易日和同一研究身份只允许一次成功执行，身份变化或人工操作仍允许重算。研究统计区分可执行、持仓市值、最终可退出和净收益样本，并单独报告真实延迟比例。

## Milestone 12.8 Research 边界正确性（2026-09-25）

Research V4 区分停牌、非活跃和普通行情缺失，自动刷新完整覆盖 Next Open、60 日 Horizon 与 5 日退出搜索。明确停牌日的 MFE/MAE 使用前一有效收盘价估值，其他缺失仍保持未知。研究工作台增加最终退出未解决率，用于识别搜索窗口结束后仍占用资金的尾部样本。

## Milestone 12.8.1 Research 正确性收尾（2026-09-25）

Research V5 统一入场与退出的状态优先级，并将最终退出区分为成功、完整观察后未解决、尚待观察三种状态。自动 Research 必须确认整个回刷窗口的生产分析完整，窗口中间存在缺口时不得启动替换式写入。目标日处于退市日之后时不得沿用退市前最后成交价；具体退市结算与损失规则由 M13 Execution Rule 定义。
