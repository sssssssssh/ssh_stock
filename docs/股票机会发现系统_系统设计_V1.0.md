...
`expected_theme_codes_on_date()` 使用 `list_date <= trade_date`、`first_seen_date <= trade_date`、`last_seen_date >= trade_date`（可空起始字段不限制），不以当前 `is_active` 过滤。Catalog 观察日使用实际运行日，不用历史 Backfill 结束日伪装观察日；发现消失时不覆盖 `last_seen_date`。Theme Daily 和可选源只在可信质量状态下对账；CatchUp 在 Core Raw 后独立修复 Theme Raw，成功再触发向后重算。Moneyflow rolling 在计算前过滤非可信日期，并用交易日历验证连续三日。Left 首次信号比较上一真实交易日 state/score；生命周期阈值来自 opportunity 配置。`ths_member` 数值 Safe Limit 待官方或代理单次行数上限证据，不得猜测。
