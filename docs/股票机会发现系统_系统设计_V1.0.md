...
- `dirty_repair` 模式会读取全部 OPEN dirty range，从最早 dirty_start_date 重算到最新 `stock_daily.trade_date`，完成后标记 RESOLVED，失败标记 FAILED。
...
- Catch-up 的分析完整性由 `analysis_complete_dates()` 判断，最小条件为 `stock_factor_daily`、`market_daily`、`sector_factor_daily` 和当前 `algo_version` 的 `stock_state_daily` 同日存在；不得只用 `MAX(stock_state_daily.trade_date)` 作为完整依据。
- Raw 完整但分析缺失时，Catch-up 创建 `recalculate` 任务并复用 `app.services.recalculation.run_recalculation()`，只补计算，不访问 Tushare。
...
- Raw-only refresh 之后如果存在 OPEN dirty range，Scheduler 自动执行 Dirty Repair，从最早 dirty start 到最新 Raw 交易日；成功 `RESOLVED`，失败 `FAILED`。
