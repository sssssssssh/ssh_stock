...
退出校验顺序为 Status 完整性、停牌、非活跃、Raw 完整性及价格/跌停规则；`_stock_rows()` 同步加载 `is_active`。Scheduler 用 `research_refresh_lookback()` 计算配置值与语义下限的较大值。股票极值窗口只在 `status_present=true AND is_suspended=true` 时以最近有效 `adj_close` 填充 high/low，普通缺失使 MFE/MAE 保持 NULL。Analytics 以成功入场数为分母生成 final exit success/unresolved 指标。Realtime negative cache 对当日与最近一日取正常 TTL 和负缓存 TTL 的较小安全值。
