...
- `app.jobs.daily_job`：单日完整日更任务，保留拉取和计算的一体化收盘后流水线。
- `app.jobs.backfill_job`：历史区间原始行情同步任务，只写入交易日历、日线、复权因子、每日指标和指数日线。
...
- `BackfillJob` 新增 `_raw_data_complete()` 检查，重跑时对 `stock_daily`、`stock_adj_factor`、`stock_daily_basic`、`index_daily` 均有记录且质量非 ERROR 的交易日执行跳过。
...
- `mark_dirty_ranges_failed()` 的状态语义改为失败后恢复 `OPEN`，并记录重试次数和最后错误；`dirty_repair` 再次触发时仍能选中该 range。
