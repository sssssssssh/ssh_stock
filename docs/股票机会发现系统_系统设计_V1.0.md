...
- `app.jobs.catchup_job`：Scheduler 触发时使用的自动补漏编排器。它同步近期交易日历，读取最新 `stock_state_daily` 日期和近期 RawCompleteness 状态，生成少量缺口补跑计划，然后逐日复用 `DailyJob`。
...
本轮未新增表和迁移；Raw 数据层仍按 Milestone 8 范围封版。
