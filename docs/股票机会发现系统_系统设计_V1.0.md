...
## Milestone 12.6 工程收口设计（2026-09-25）

`core/strategy_config.py` 使用嵌套 Pydantic 模型表达完整 Strategy YAML，并统一采用 `extra=forbid`。`core/config.py` 在生成分析身份前完成校验，再输出普通字典以保持既有调用接口。认证限流、Provider Gateway 和服务心跳已形成独立边界；ORM 模型继续集中管理，避免仅为拆文件引入元数据加载顺序风险。CI 后端覆盖率门槛设为 70%，前端以 V8 provider 输出覆盖率；Docker 最终阶段创建并切换至 `appuser`。

## Milestone 12.7 Research 正确性设计（2026-09-25）

`_validated_stock_exit()` 是股票 Horizon 与最终退出的共同校验入口，为 M13 Execution Rule 保留单一演进边界。Mark Resolver 从目标日向 Entry Date 回扫 `adj_close`，迁移 `0025_m12_7_research_correctness` 在股票和题材 Forward Eval 增加 `mark_trade_date5/10/20/60`。`future_dates()` 的查询上限为 `batch size + 1 + max horizon + delayed search days`。Scheduler 从成功任务 JSON 身份和目标日期判断自动运行幂等；Realtime 缺口确认后才构造 Provider，空结果使用独立 TTL；Retention 删除 30 天前服务实例心跳。

## Milestone 12.8 Research 边界设计（2026-09-25）

入场与退出统一按 Status 完整性、停牌、非活跃、Raw 完整性及价格/涨跌停规则校验；`_stock_rows()` 同步加载 `is_active` 和 `delist_date`。Scheduler 用 `research_refresh_lookback()` 计算配置值与语义下限的较大值，并复用 `analysis_complete_dates()` 验证整个回刷窗口，窗口存在中间缺口时不得启动 replace-slice。股票极值窗口只在 `status_present=true AND is_suspended=true` 时以最近有效 `adj_close` 填充 high/low，普通缺失使 MFE/MAE 保持 NULL；目标日晚于 `delist_date` 时 Mark Resolver 返回未知，不把退市前价格当作停牌价格无限延用。Forward Eval 记录 delayed-exit window maturity；Analytics 将最终退出区分为 SUCCESS、UNRESOLVED 与 PENDING，成功率和未解决率只以已完成观察样本为分母。Realtime negative cache 对当日与最近一日取正常 TTL 和负缓存 TTL 的较小安全值。

M12.8.2 将 `final_exit_status5/10/20/60` 持久化到股票与题材 Forward Eval。状态空间固定为 SUCCESS、PENDING、UNRESOLVED、DATA_INCOMPLETE：只有 SUCCESS 与 UNRESOLVED 组成完成观察样本，数据缺口不再视为流动性失败。Analytics Selector 必须同时读取 `delayed_exit_window_matureN` 与 `final_exit_statusN`。Theme Research 只对 Raw 状态为 PASS/WARNING 且当前身份 ThemeFactor 存在的日期执行 replace-slice，其余日期非破坏性跳过并记录 `THEME_SOURCE_INCOMPLETE`。

M12.8.3 将 Research 身份升级为 `research_eval_v7`：延迟退出路径中的数据缺口优先于 PENDING 和后续 SUCCESS，并清空该 horizon 的 delayed exit 结果；已知停牌、跌停、非活跃仍可继续搜索。Opportunity ready gate 读取 `opportunity_vs_state`，Theme ready gate 同时要求 `ths_theme_daily` 可用及 `theme_factor_vs_theme_daily` 覆盖质量非 ERROR。跳过日期不进入 replace-slice，业务筛选为空则仍可正常替换。两个 Forward Eval 表的 8 个最终状态列由数据库 CHECK 约束限定合法状态空间。

M12.8.4 将 Research 身份升级为 `research_eval_v8`，不新增数据库迁移。CatchUp 与 Scheduler 共用 `analysis_readiness` 服务，核心完整性固定要求当前身份的 factor/market/sector/state 覆盖通过。Opportunity Research 必须先通过核心门禁，再要求 state 非空以及 `opportunity_vs_state` 为 PASS/WARNING；Theme Research 必须先通过核心门禁，再独立要求 `ths_theme_daily` 与 `theme_factor_vs_theme_daily` 为 PASS/WARNING，不依赖 Opportunity 覆盖结果。所有跳过日期均排除在 replace-slice 之外；就绪日期即使业务筛选结果为空，也会权威替换为空。

M12.8.5 将 Research 身份升级为 `research_eval_v9`，不新增数据库迁移。Sector expected set 不使用 `Sector.is_active` 总数，而是将当天当前 Factor identity 中 `eligible=true` 的股票与 `valid_from <= D <= valid_to`（或 `valid_to IS NULL`）的 SectorMember 连接并取 distinct sector_id。Actual set 仅来自当天当前 Sector calc/config identity；matched/expected 按 0.98/0.90 阈值产生 PASS/WARNING/ERROR，expected 为空时 fail-closed。Production quality 将结果持久化为 `sector_factor_vs_expected`，Research Core Readiness 仅接受 PASS，因此不完整日期不会进入 Opportunity/Theme replace-slice，也不会删除已有 v9 结果。

M12.8.6 将 Research 身份升级为 `research_eval_v10`，不新增数据库迁移，也不为 ResearchTransitionEval 增加 eval_version 列。Transition ready gate 要求事件日及其真实前一开市日均通过 Opportunity source readiness，并要求事件日后已经发生、最多 20 个开市日内的每个日期通过 Core Readiness；尚未发生的日期不阻断。只有 ready_dates 能进入 source query、scope filters 和 replace-slice，跳过日期已有结果保持不变，ready 但无事件的日期仍可权威清空。Transition identity 中的 `research_config_hash` 包含 eval_version，因此 v9/v10 自动隔离。Sector coverage 在匹配率达到 PASS 但存在 extra sector 时返回 WARNING，Research Core 继续只接受 PASS。
