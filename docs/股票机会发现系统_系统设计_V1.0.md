...
- 前端首页“数据覆盖”面板已接入 `/api/v1/system/data-coverage`，用于查看已拉取交易日和各表行数。
- 前端首页“数据覆盖”面板已新增日期范围输入和“拉取数据”按钮，直接调用 `/api/v1/jobs/backfill`。
- 前端首页“数据覆盖”面板已新增任务状态区，轮询 `/api/v1/jobs` 展示当前任务、最近任务、进度条、步骤、当前交易日和行数。
